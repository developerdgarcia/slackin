...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe }){
  let div = dom('.splash',
    ...
    dom('form id=invite',
      ...
      dom('input.form-item type=email name=email placeholder=seu@email.com '
        + (!iframe ? 'autofocus' : '')),
      coc && dom('.coc',
        dom('label',
          dom('input type=checkbox name=coc value=1'),
          'I agree to the ',
          dom('a', { href: coc, target: '_blank' }, 'Termos de Serviço'),
          '.'
        )
      ),
    ...
    dom('p.signin',
      'or ',
      dom(`a href=https://${org}.slack.com target=_top`, 'Se Cadastrar Aqui'),
      '.'
    ),
    !iframe && dom('footer',
      'powered by ',
